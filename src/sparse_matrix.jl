...
"""
    ED_Hmlt_Matrix(sorted_mbs_block_list, sorted_onebody_scat_list, sorted_twobody_scat_list)
...
# Returns  
- `ExtendableSparseMatrix{ComplexF64}`: Sparse Hamiltonian matrix (Hermitian)
...
"""
    ED_Hmlt_Matrix_threaded(sorted_mbs_block_list, sorted_onebody_scat_list, sorted_twobody_scat_list)

Threaded version of ED_Hmlt_Matrix with pre-computed state mapping and COO format construction.

This function provides the same functionality as ED_Hmlt_Matrix but uses multi-threading
